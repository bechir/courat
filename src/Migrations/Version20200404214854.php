 ...
 */
final class Version20200404214854 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('ALTER TABLE info ADD COLUMN created_at DATETIME DEFAULT NULL');
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__article AS SELECT id, subject_id, classe_id, category_id, title, url, size FROM article');
        ...
        $this->addSql('INSERT INTO article (id, subject_id, classe_id, category_id, title, url, size) SELECT id, subject_id, classe_id, category_id, title, url, size FROM __temp__article');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__article AS SELECT id, subject_id, classe_id, category_id, title, url, size FROM article');
        $this->addSql('DROP TABLE article');
        $this->addSql('CREATE TABLE article (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, subject_id INTEGER NOT NULL, classe_id INTEGER NOT NULL, category_id INTEGER NOT NULL, title VARCHAR(255) NOT NULL, url VARCHAR(255) NOT NULL, size VARCHAR(255) DEFAULT NULL)');
        $this->addSql('INSERT INTO article (id, subject_id, classe_id, category_id, title, url, size) SELECT id, subject_id, classe_id, category_id, title, url, size FROM __temp__article');
        $this->addSql('DROP TABLE __temp__article');
        $this->addSql('CREATE INDEX IDX_23A0E6623EDC87 ON article (subject_id)');
        $this->addSql('CREATE INDEX IDX_23A0E668F5EA509 ON article (classe_id)');
        ...
        $this->addSql('CREATE INDEX IDX_169E6FB923EDC87 ON course (subject_id)');
        $this->addSql('CREATE INDEX IDX_169E6FB9953C1C61 ON course (source_id)');
        $this->addSql('CREATE TEMPORARY TABLE __temp__info AS SELECT id, title, link, filename, updated_at FROM info');
        $this->addSql('DROP TABLE info');
        $this->addSql('CREATE TABLE info (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title VARCHAR(255) NOT NULL, link VARCHAR(255) NOT NULL, filename VARCHAR(255) DEFAULT NULL, updated_at DATETIME DEFAULT NULL)');
        $this->addSql('INSERT INTO info (id, title, link, filename, updated_at) SELECT id, title, link, filename, updated_at FROM __temp__info');
        $this->addSql('DROP TABLE __temp__info');
        $this->addSql('DROP INDEX IDX_D499BFF69C24126');
        $this->addSql('CREATE TEMPORARY TABLE __temp__planning AS SELECT id, day_id FROM planning');
        $this->addSql('DROP TABLE planning');
